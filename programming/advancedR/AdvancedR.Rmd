...
Actually, the ancestors of every environment will eventually lead back to the empty environment. You can see all ancestors with the `env_parents()` function:
...
`env_parents()` stops when it gets to the global environment; the global environment's ancestors include every attached package, so this provides the most useful information. But we can always override this default behavior by adding an extra argument to the function call.
...
ADD EXPLANATION OF SPECIAL ENVIRONEMNTS
