...
Do you have an _outcome_ (or _response_) variable that you wish to model (or _predict_) as a function of one or more _inputs_ (or _covariates_)? If so, you need some regression.

__Notation:__

- $y =$ our __outcome__ variable.

- $\mathbf x =$ our __input__ variables. Bold face indicates a _vector_.

- $\boldsymbol \beta =$ our parameters.

- $\mathbf x^\top \boldsymbol \beta = \beta_0 + \beta_1 x_1 + \dots + \beta_p x_p$ is the __linear predictor__ (also called $\boldsymbol \eta$).
...
### Model Fit: Case study

Both linear and logistic regression can be fit using _maximum likelihood estimation_.

Two additionl jargon terms:

1. __Likelihood__: The probability of your data, given the parameters. You want to make it as big as possible.

2. __Deviance__: The distance between data and fit. You want to make this as small as possible.

The likelihood function for logistic regression 


Another big _mathematical_ assumption is that observations are independent of each other. 

$$
\prod_{i=1}^n \Pr(y_i \mid \mathbf x_i) = \prod_{i=1}^n p_i^{y_i} (1 - p_i)^{1 - y_i}
$$



$$
\sum_{i=1}^n \log \bigg(\Pr(y_i \mid \mathbf x_i)\bigg) = \sum_{i=1}^n \log \bigg(p_i^{y_i} (1 - p_i)^{1 - y_i}\bigg)
$$
...
log_likelihood <- function(beta) {
  eta <- X %*% beta
  p <- 1 / (1 + exp(-eta))
  return(sum(dbinom(y == 1, size = 1, prob = p, log = TRUE)))
}

opt <- optim(rep(0, ncol(X)), fn = log_likelihood, method = "BFGS",
...
>If we sant to do _linear_ regression and it appears that noise is increasing over time, we likely need a transformation. Passenger numbers are like sales volume—they are always positive and are often discussed in percentage terms. This suggests that we should be working on the log scale.
...
airline$pred <- predict(model_time) %>% exp()
```
...
airline <- airline %>% 
  ...
  na.omit()   ## this will remove one observation from our dataset
