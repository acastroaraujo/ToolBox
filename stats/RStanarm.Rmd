...
The 95% confidence intervals for `dist` are as follows:
...
- `posterior_linpred()` yields simulations of the possible values of $\boldsymbol \eta = \alpha + \beta \mathbf x$, with variation coming exclusively from the posterior uncertainty of the coefficients.
...
dim(yrep) ## rows = draws, columns = observations
```
...
CHANGE THIS EXAMPLE SO THAT IT MAKES SENSE.

Suppose we are interested in predicting the election outcomes when growth is 2%. We simply do as follows:

```{r}
new_df <- tibble(dist = 2)
ypred <- posterior_linpred(logistic_bayes, newdata = new_df)
...
```{r, fig.width=8, fig.height=2, message=FALSE}
ggplot(NULL, aes(x = logistic_bayes$stanfit@sim$samples[[1]]$mean_PPD)) +
  geom_histogram(fill = "steelblue1", color = "black") +
  geom_vline(xintercept = mean(wells$switch), size = 3, color = "tomato") +
  labs(x = "mean_PPD")
```


```{r, fig.width=8, fig.height=2, message=FALSE}
bayesplot::ppc_stat(y = wells$switch, 
